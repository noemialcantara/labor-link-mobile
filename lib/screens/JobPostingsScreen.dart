...
class _JobPostingsScreenState extends State<JobPostingsScreen> {
  ...
  _getSubscriptionPlan() {
    SubscribersApi.getUserPlan(FirebaseAuth.instance.currentUser?.email ?? "")
        .then((QuerySnapshot querySnapshot) {
      if (querySnapshot.docs.length > 0) {
        if (querySnapshot.docs.first.get("is_cancelled") == "false" &&
            DateTime.parse(
                  ...
                  itemBuilder: (ctx, index) => GestureDetector(
                      onTap: () {
                        ...
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (_) => JobPostingManagementScreen(
                                      companyName: companyName,
                                      job: jobData,
                                    ))).then((_) => _getJobPostingsCount());
                      },
                      child: Container(
                          margin:
                                      ...
                                      SizedBox(
                                          ...
                                          child: Padding(
                                              padding:
                                                  EdgeInsets.only(left: 10),
                                              child: Text(
                                                "${snapshot.data?.docs[index].get("required_applicant_count")} applicants",
                                                maxLines: 1,
                                                softWrap: false,
                                                overflow: TextOverflow.ellipsis,
                                                style: GoogleFonts.poppins(
                                              ...
                                              child: RichText(
                                                ...
                                                text: TextSpan(
                                                    ...
                                                    children: [
                                                      TextSpan(
                                                          text:
                                                              '${snapshot.data?.docs[index].get("min_salary").toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')} /mo',
                                                          style: GoogleFonts
                                                              .poppins(
                                                                  fontSize:
                                                                      14.0,
                                ...
                                Container(
                                    ...
                                    decoration: BoxDecoration(
                                        color: snapshot.data?.docs[index]
                                            ...
                                            : snapshot.data?.docs[index].get(
                                                ...
                                                : Color(0XFFdaffe3),
                                        border: Border.all(
                                            color: snapshot.data?.docs[index].get(
                                                        "employment_type") ==
                                                    "Freelance"
                                                ? Color(0xfffcf5ed)
                                                : snapshot.data?.docs[index].get(
                                    ...
                                    child: SizedBox(
                                        ...
                                        child: Padding(
                                            ...
                                            child: Text(snapshot.data?.docs[index].get("employment_type"),
                                                maxLines: 1,
                                                overflow: TextOverflow.ellipsis,
                                                ...
                                                style: GoogleFonts.poppins(
                                                    fontSize: 14.0,
                                                    fontWeight: FontWeight.w600,
                                                    color: snapshot.data?.docs[index].get("employment_type") == "Freelance"
                                                        ? Color(0xffC86C38)
                                                        : snapshot.data?.docs[index].get("employment_type") == "Full-time"
                                                            ? Color(0xff5386E4)
                                                            : Color(0xff2BC155)))))),
                                Container(
          ...
          onPressed: () {
            ...
            if (!isAlreadySubscribed && jobPostingsCount >= 10) {
              AwesomeDialog(
                ...
                title: 'Alert!',
                desc:
                    'You are currently in a free plan wherein you only have 10 free job postings. Please subscribe to our monthly or annualy plan to post unlimited jobs in our application.',
                btnOkOnPress: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
              ...
              Navigator.push(
                      ...
                      MaterialPageRoute(
                          builder: (_) => JobPostingManagementScreen(
                              companyName: companyName)))
                  .then((value) => _getJobPostingsCount());
            }
          },
        ...
        body: SingleChildScrollView(
            child: Column(
              ...
              Padding(
                padding: EdgeInsets.only(left: 30, right: 30),
                child: Text(
                  'You have $jobPostingsCount Job Posting(s)',
