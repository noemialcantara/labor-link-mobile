...
class _JobApplicationScreenState extends State<JobApplicationScreen> {
  ...
  void submitApplication() async {
    ...
    if (selectedResume != "") {
      JobApplication payload = JobApplication(
          selectedResumeProfileName,
          selectedResume,
          coverLetter,
          widget.jobDetails.jobId,
          userEmail,
          widget.jobDetails.companyLogo,
          ...
          "Reviewing",
          widget.jobDetails.employmentType,
          formattedDateNow);
      JobApplicationApi.submitApplication(payload.toJson());

      await FirebaseChatApi.addChatUserWithFrom(employerEmailAddress, userEmail)
          .then((value) {
        FirebaseChatApi.sendFirstMessageCustom(
            value["from_id"],
            value["to_id"],
            "Hello ${selectedResumeProfileName},\n\nThis is to inform you that we have already received your application regarding ${widget.jobDetails.jobName} position. We are now currently reviewing your resume. Rest assured that we will send you a message regarding the interview schedule.\n\nThank you.",
            Type.text);
      });

      createNotification(userEmail, "New Activity",
          "${widget.jobDetails.companyName} has already received your application for ${widget.jobDetails.jobName} position.");
      createNotification(employerEmailAddress, "New Activity",
          "$selectedResumeProfileName has submitted his/her application for ${widget.jobDetails.jobName} position.");

      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (_) => JobApplicationSuccessScreen(
  ...
  showAlertDialog(BuildContext context) {
    ...
    AlertDialog alert = AlertDialog(
      ...
      content:
          Text("Please select a resume first", style: GoogleFonts.poppins()),
      actions: [
        okButton,
      ],
    );
          ...
          switch (snapshot.connectionState) {
            ...
            case ConnectionState.done:
                                      ...
                                      Transform.scale(
                                          ...
                                          child: Theme(
                                              ...
                                              child: Checkbox(
                                                ...
                                                value:
                                                    isResumeCheckedList[index],
                                                shape: CircleBorder(),
                                                onChanged: (bool? value) {
                                                  print(
                                                      "THE SELECTED RESUME IS ${snapshot.data?.docs[index].get("link")}");
                                                  setState(() {
              ...
              return Padding(
                  padding: EdgeInsets.only(left: 20, right: 20),
                  child: Text(
                    "There is no resume yet. Please upload a resume first.",
                    style: GoogleFonts.poppins(
                        fontSize: 16, color: Color(0xff95969D)),
                  ));
          }
        });
  }
            ...
            Row(
              children: [
                Padding(
                    ...
                    child: Image.network(widget.jobDetails.companyLogo,
