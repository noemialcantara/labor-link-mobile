...
class CertificationApi {
  ...
  static getCertificationData(String emailAddress) {
    return firestore
        .collection('certifications')
        .where("email_address", isEqualTo: emailAddress)
        .get();
  ...
  static uploadID(PlatformFile file, String emailAddress) async {
    ...
    await FirebaseStorage.instance
        .ref('certifications/${file.name}')
        .putFile(convertedFile);

    final Reference ref =
        FirebaseStorage.instance.ref().child('certifications/${file.name}');
    String link = await ref.getDownloadURL();

    Certification payload = Certification(file.name, link, emailAddress);
    firestore.collection('certifications').add(payload.toJson());
  }
  ...
  static deleteIDByLinkId(String linkId) async {
    firestore
      ...
      firestore.collection('certifications').doc(id).delete();
    });
