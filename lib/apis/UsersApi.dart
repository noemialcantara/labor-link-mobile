...
class UsersApi {
  ...
  static createUser(bool isApplicantFirstMode, Map<String, dynamic> user) {
    if (!isApplicantFirstMode) {
      firestore.collection('employers').add(user);
    } else {
      firestore.collection('applicants').add(user);
    }
  }
  ...
  static Future<bool> urgentlyCreateUser(String emailAddress) async {
    ...
    return await firestore
      ...
      if (applicantCount == 0) {
        firestore.collection('applicants').add(userPayload);
      }
  ...
  static updateUserProfile(Map<String, dynamic> userData) {
    firestore
        .collection('applicants')
        .where("email_address", isEqualTo: userData["email_address"])
        .snapshots()
        .first
        .then((value) {
      String id = value.docs.first.id.toString();
      firestore.collection('applicants').doc(id).update(userData);
    });
  }

  static updateEmployerProfile(Map<String, dynamic> userData) {
    firestore
        ...
        .then((value) {
      String id = value.docs.first.id.toString();
      firestore.collection('employers').doc(id).update(userData);
    });
  }
  ...
  static Stream<QuerySnapshot<Map<String, dynamic>>> streamEmployerDataByEmail(
      String email) {
    return firestore
        .collection('employers')
  ...
  static Stream<QuerySnapshot<Map<String, dynamic>>> streamApplicantDataByEmail(
      String email) {
    return firestore
        .collection('applicants')
        .where("email_address", isEqualTo: email)
